...
const CreateExamScheduleModal = ({ slotData, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    course_id: '',
    section: '',
    // ── slotData.date is already "YYYY-MM-DD" from toLocalDateStr() in
    //    ExamRoutine.jsx — safe to use directly as the date input value.
    //    Never wrap it in new Date() here.
    date: slotData.date,
    start_time: '08:00',
    end_time: '08:30',
    building_name: '',
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // ── Build payload explicitly so we control exactly what is sent.
      //    formData.date is "YYYY-MM-DD" (from the date input value).
      //    input[type="date"].value is always "YYYY-MM-DD" — never convert
      //    it to a Date object or call toISOString() on it.
      const payload = {
        course_id:        formData.course_id,
        section:          formData.section,
        date:             formData.date,        // "YYYY-MM-DD" string  ✅
        start_time:       formData.start_time,  // "HH:MM" string       ✅
        end_time:         formData.end_time,    // "HH:MM" string       ✅
        building_name:    formData.building_name,
        room:             formData.room,
        exam_description: formData.exam_description,
      };

      await api.createExamSchedule(token, payload);
      alert('Exam scheduled successfully!');
      onSuccess();
    } catch (err) {
      alert('Failed to create: ' + err.message);
    ...
    <div className="update-modal-overlay" onClick={onClose}>
      <div className="update-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* ── Date is pre-filled from slotData and shown read-only.
                The user already picked the date by clicking that day's
                "+ Add Exam" button in ExamRoutine.jsx. Showing it as
                a disabled input prevents accidental edits and makes it
                clear which day they are scheduling for.
                If you want to allow changing the date, switch
                readOnly/disabled to a normal input — the value from
                input[type="date"] is always "YYYY-MM-DD", so it stays safe. */}
          <div className="form-group">
            <label>Date</label>
            <input
              type="date"
              name="date"
              value={formData.date}
              onChange={handleChange}  // allow changing if needed
              required
            />
          </div>
